...
public class VidaExtra extends Entity {
    ...
    public VidaExtra(int x, int y, int width, int height, BufferedImage sprite) {
    ...
    public boolean colisao(int nextX, int nextY){
        Rectangle leVidaExtra = new Rectangle(nextX + maskx, nextY + masky, maskw, maskh);
        for (int i = 0; i < Game.entidades.size(); i++) {
            Entity entidade = Game.entidades.get(i);
            if (entidade instanceof Solido){
                Rectangle solido = new Rectangle(entidade.getX() + maskx, entidade.getY() + masky, maskw, maskh );
                if (leVidaExtra.intersects(solido)){
                    return true;
                }
            }
        }
